...
module.exports = env => {
    ...
    //     if (!!development && /src\/bluetooth/.test(path)) {
    //         console.log('replacementFunc', path, nsWebpack.getResolver([platform, 'tns'])(path));
    //         // return resolve(srcFullPath, 'bluetooth.d.ts')
    //     }
    //     return nsWebpack.getResolver([platform, 'tns'])(path)
    // }
    const ngCompilerPlugin = new AngularCompilerPlugin({
        hostReplacementPaths: nsWebpack.getResolver([platform, 'tns']),
        platformTransformers: ngCompilerTransformers.map(t => t(() => ngCompilerPlugin)),
        mainPath: resolve(appPath, entryModule),
        tsConfigPath: join(__dirname, `tsconfig.${platform}.json`),
    ...
    const config = {
        ...
        module: {
            rules: [
                ...
                {
                    test: /[\/|\\]app\.scss$/,
                    use: ['nativescript-dev-webpack/style-hot-loader', { loader: 'css-loader', options: { minimize: false, url: false } }, 'sass-loader']
                },
