...
export class TNS_BluetoothGattCallback extends android.bluetooth
  ...
  onInit(owner: WeakRef<Bluetooth>) {
    ...
    CLog(
      CLogTypes.info,
      `TNS_BluetoothGattCallback.onInit ---- this.owner: ${this.owner}`
    );
  }
  ...
  onConnectionStateChange(
    ...
    if (
      ...
      CLog(
        CLogTypes.info,
        "TNS_BluetoothGattCallback.onConnectionStateChange ---- discovering services -----"
      );
  ...
  onCharacteristicRead(
    ...
    CLog(
      CLogTypes.info,
      `TNS_BluetoothGattCallback.onCharacteristicRead ---- gatt: ${gatt}, characteristic: ${characteristic}, status: ${status}, device: ${device}`
    );
    const stateObject = this.owner.get().connections[device.getAddress()];
    if (!stateObject) {
      this.owner.get().gattDisconnect(gatt);
